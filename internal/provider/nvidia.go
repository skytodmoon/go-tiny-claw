...
// NvidiaProvider NVIDIA API Provider
type NvidiaProvider struct {
	client openai.Client
	model  string
}

// NewNvidiaProvider 创建 NVIDIA Provider
func NewNvidiaProvider(model string) *NvidiaProvider {
	apiKey := os.Getenv("NVIDIA_API_KEY")
	if apiKey == "" {
		panic("请设置 NVIDIA_API_KEY 环境变量")
	}
...
func (p *NvidiaProvider) Generate(ctx context.Context, msgs []schema.Message, availableTools []schema.ToolDefinition) (*schema.Message, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("NVIDIA API 请求失败: %w", err)
	}
	if len(resp.Choices) == 0 {
		return nil, fmt.Errorf("NVIDIA API 返回了空的 Choices")
	}
