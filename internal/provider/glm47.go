...
type GLM47Provider struct {
	client openai.Client
	model  string
}

func NewGLM47Provider(model string) *GLM47Provider {
...
func (p *GLM47Provider) Generate(ctx context.Context, msgs []schema.Message, availableTools []schema.ToolDefinition) (*schema.Message, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("GLM4.7/NVIDIA API 请求失败: %w", err)
	}
