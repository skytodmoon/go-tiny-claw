...
- ✅ **Session 架构**：工作区跟随会话动态绑定，支持多用户/多终端隔离
- ✅ **内存压缩器（Compactor）**：迭代压缩上下文，确保低于阈值，防止 OOM
...
框架引入 Session 概念，实现工作区跟随会话动态绑定：

**核心设计**：
- **会话隔离**：每个 Session 绑定独立的工作区（WorkDir）
- **工作记忆**：通过 `GetWorkingMemory(N)` 截取最近 N 条消息作为短期记忆
- **并发安全**：使用读写锁保护会话历史，支持多协程访问
- **上下文组装**：System Prompt + Working Memory 动态组合

**关键特性**：
- **多用户隔离**：不同用户会话完全独立，数据互不干扰
- **动态绑定**：工作区在会话创建时指定，支持灵活部署
- **持久化预留**：预留了持久化接口，便于后续扩展

### 内存压缩器（Compactor）

框架集成智能内存压缩器，防止上下文过长导致 OOM：

**压缩策略**：
| 压缩轮次 | 保留字符数 | 说明 |
|----------|------------|------|
| 第1轮 | 1000 | 温和压缩，保留主要信息 |
| 第2轮 | 600 | 更激进，压缩 Working Memory |
| 第3轮及以后 | 300 | 非常激进，确保低于阈值 |

**压缩规则**：
- **远期历史**：工具输出被掩码化（替换为摘要）
- **短期记忆**：超大内容被掐头去尾（保留前后各 N 字符）
- **系统提示词**：绝对保留，不做任何修改

**优势**：
- **自动迭代**：单次压缩不够时自动进行多轮迭代
- **智能保护**：区分远期历史和短期记忆，保留关键信息
- **阈值保证**：确保最终上下文长度低于设定阈值
...
│   ├── context/                 # 上下文管理
│   │   ├── compactor.go         # 内存压缩器（OOM保护）
│   │   └── composer.go          # Prompt 组装器
│   ├── tools/                   # 工具注册表与内置工具
│   │   ├── registry.go         # 工具注册表
│   │   ├── read_file.go       # 文件读取工具
